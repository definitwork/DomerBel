import openpyxl
from advertisement.models import Advertisement, Category, Region, Field, Spisok, ElementTwo, PhotoAdvertisement
...
def check_article(ads,advertisements):
    ...
    if 'артикул' in ads:
        ...
        if article == None:
            ...
            if article in articals: 
                return f'Уже сществует обявление с таким артикулом {article}'
            else:
                return True
...
def check_additional_information(ads,user):
    ...
    if 'категория' in ads:
        ...
        for field_from_excel in ads.keys():
            '''!!!!!Исключить из списка тип дивгателя и добавить его в базу данных'''
...
def update_photo(ads_for_save,file_name,uploud_zip,email):
    ...
    try:
        ...
        # preview_image = shutil.move(f'./{image_from_zip}',f'./media/{new_location_image}')
        preview_image = os.replace(f'./{image_from_zip}',f'./media/{new_location_image}')
        print(' функци update_image отработала до конца нормально!')
...
def save_many_ads_from_excel(uploud_file, user):
    ...
    row_in_excel = 2
    list_ads_error = []
    value_author = User.objects.get(id=user.id)
    ...
    while row_in_excel <= sheet._max_row:
        ...
        if len(ads) != 0:
            status_ads = []
            value_artical = check_article(ads,advertisements)
            ...
            json_file = check_additional_information(ads,user)
    ...
    if len(list_ads_error) != 0:
        error_file = write_file_with_error_ads(list_ads_error,user.email)
        '''тут можно передавать список ошибок'''
...
def save_many_ads_from_zip(uploud_zip, user):
    '''Функция для сохранениея обявлений из электронного архива'''
    print('Функция save_many_ads_from_zip')
    list_ads_error = []
    value_author = User.objects.get(id=user.id)
    advertisements = Advertisement.objects.filter(author = value_author)
    with ZipFile(uploud_zip,'r') as zip:
        for file in zip.namelist():
            if file.endswith('.xlsx'):
                excel = zip.extract(file,path=f'./media/files_for_bulk_import_of_ads/{user.email}/')
    ...
    while row_in_excel <= sheet._max_row:
        ...
        if len(ads) != 0:
            ...
            if len(status_ads) ==  0:
                ...
                description = str(ads.get('описание')))
                ads_for_save.save()
                preview_image = update_photo(ads_for_save,ads.get('фото1'),uploud_zip,user.email)
                if preview_image != False:
                    ...
                    for key in ads.keys():
                        if key.startswith('фото') and key != 'фото1':
                            try:
                                photo = update_photo(photo_save,ads.get(key),uploud_zip,user.email)
                                photo_save = PhotoAdvertisement(
                                    photo = photo,
                                    advertisement = ads_for_save
                                )
                                ...
                                list_ads_error.append(ads)
                                print('ошибка в изображениии')
                                break
                else:
                    ads_for_save.delete()
                    status_ads.append(f'Невозможно сохранить файл {ads.get("фото1")}')
                    list_ads_error.append(ads)
                    print('ошибка в превью фото')
                    break
            else:
                print('Ошибка где-то в полях обявления')
                list_ads_error.append(ads)
        row_in_excel = row_in_excel + 1
    if len(list_ads_error) != 0:
        error_file = write_file_with_error_ads(list_ads_error,user.email)
